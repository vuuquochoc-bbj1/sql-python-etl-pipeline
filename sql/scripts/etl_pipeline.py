...
query = """
    SELECT Id, OrderCode, Quantity, TotalAmount, Status 
...
df['OrderType'] = np.where(df['Quantity'] >= 10, 'Bulk', 'Retail')

df['OrderID'] = df['Id'] - 19471
#Row ID start from 19472 and increase so I delete by 19471 to get Row ID starting 1
summary_df = df.groupby('OrderType')['TotalAmount'].sum().reset_index()
...
# 4. Display result
print("\n--- REVENUE SUMMARY REPORT BY ORDER TYPE ---")
print(summary_df)
